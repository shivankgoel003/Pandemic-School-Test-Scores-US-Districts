...
state_colors <- rainbow(length(unique(merged_data$state)))

# Create a scatter plot with colored points
plot(merged_data$average_students_per_teacher, merged_data$average_pass, 
     xlab = "Average Students per Teacher", ylab = "Average Pass Rates",
     main = "Relationship Between Average Students per Teacher and Average Pass Rates",
     col = state_colors[as.factor(merged_data$state)], pch = 19)

# Add labels for each point
text(merged_data$average_students_per_teacher, merged_data$average_pass, labels = merged_data$state, pos = 3, cex = 0.8)

# Fit a linear regression model
lm_model <- lm(average_pass ~ average_students_per_teacher, data = merged_data)

# Add a trend line
abline(lm_model, col = "red")

# Add a legend
legend("bottomright", legend = unique(merged_data$state), col = state_colors, pch = 19, cex = 0.8, title = "State")
